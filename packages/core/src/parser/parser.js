...
import { DEFAULT_CONFIG, ParseError, } from './types.js';
// ── Public API ────────────────────────────────────────────────────────────────
    ...
    while (idx < tokens.length) {
        ...
        switch (token.kind) {
            ...
            case 'set': {
                ...
                if (!setMatch)
                    throw new ParseError(token.line, `Invalid set directive: "${token.raw}"`);
...
function parseConfigSection(tokens, startIdx, config) {
    let idx = startIdx;
    while (idx < tokens.length) {
        const token = tokens[idx];
        if (!token)
            break;
        if (token.kind === 'script-section-header')
            break;
...
function applyConfigKey(key, value, config, line) {
...
function parseTypingSpeed(value, line) {
...
export function parseStyledText(input) {
    ...
    while (i < input.length) {
        ...
        if (braceIdx === -1) {
            ...
            if (rest.length > 0)
                result.push({ kind: 'plain', text: rest });
            break;
        }
